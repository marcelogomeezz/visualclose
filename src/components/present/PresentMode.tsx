...
export function PresentMode() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-graphite-0 flex items-center justify-center select-none">
      {/* Controls */}
      <div className={`absolute top-0 inset-x-0 h-12 px-4 flex items-center justify-between transition-opacity duration-500 z-10 ${controlsVisible ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <label className="t-label cursor-pointer hover:text-ivory">
          ...
          <button type="button" className="text-ivory hover:text-offwhite" onClick={togglePlay} title="Play / pause (space)">
            {playing ? <Icon.Pause /> : <Icon.Play />}
          </button>
          <button type="button" className="text-warm-grey hover:text-ivory" onClick={restart} title="Restart (r)">
            <Icon.Reset />
          </button>
          <button type="button" className="text-warm-grey hover:text-ivory" onClick={() => ui.exitPresent()} title="Exit (esc)">
            <Icon.Close />
          </button>
        </div>
      </div>
      ...
      <div
        ref={stageRef}
        data-aspect={aspect}
        className="present-stage relative overflow-hidden bg-graphite-0"
      ...
      >
        ...
        {/* REFERENCES */}
        {refs.length > 0 && (
          <div data-frame="references" className="absolute inset-0 bg-graphite-1">
            <div className={`absolute inset-0 flex gap-[calc(var(--u)*3)] p-[calc(var(--u)*5)] ${aspect === "16:9" ? "flex-row items-center" : "flex-col"}`}>
              <div className={`${aspect === "16:9" ? "w-[36%]" : "w-full"} shrink-0`}>
                ...
                {productPack.tagline && (
                  <div data-caption className="text-warm-grey mt-[calc(var(--u)*1.5)]" style={{ fontSize: "calc(var(--u) * 1.5)" }}>
                    {productPack.tagline}
                  </div>
                )}
              </div>
              <div className={`flex-1 min-h-0 grid content-center gap-[calc(var(--u)*1.5)] ${aspect === "16:9" ? (refs.length > 2 ? "grid-cols-2" : "grid-cols-1") : "grid-cols-2"}`}>
                {refs.map((r) => (
                  <div key={r.id} data-tile className="relative bg-graphite-0 border border-line overflow-hidden aspect-[4/3]">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={r.asset.url} alt="" className="absolute inset-0 w-full h-full object-cover" draggable={false} />
                    <span className="absolute left-[calc(var(--u)*1)] bottom-[calc(var(--u)*0.8)] t-label-strong bg-graphite-0/70 px-[calc(var(--u)*0.8)] py-[calc(var(--u)*0.3)]" style={{ fontSize: "calc(var(--u) * 0.9)" }}>
                      {r.type}
                    </span>
                  </div>
                ))}
        ...
        <div data-frame="dimensions" className="absolute inset-0 bg-graphite-1">
          <div className="absolute inset-0 p-[calc(var(--u)*6)] flex flex-col justify-center">
            ...
            <div data-caption className="t-editorial t-mono text-ivory mt-[calc(var(--u)*2)]" style={{ fontSize: `calc(var(--u) * ${aspect === "9:16" ? 5.6 : 7.2})`, letterSpacing: "-0.02em" }}>
              {aspect === "9:16" ? (
                <>
                  ...
                  <div className="text-warm-grey" style={{ fontSize: "calc(var(--u) * 2.4)" }}>
                    {dimensions.units}
                  </div>
                </>
              ) : (
            ...
            <div data-caption className="grid gap-[calc(var(--u)*0.6)] mt-[calc(var(--u)*4)] text-ivory/85" style={{ fontSize: "calc(var(--u) * 1.5)", gridTemplateColumns: aspect === "16:9" ? "repeat(3, max-content)" : "1fr", columnGap: "calc(var(--u) * 5)" }}>
              <div>
        ...
        {reality && (
          <div data-frame="compare" className="absolute inset-0">
            ...
            <div data-line className="absolute top-0 bottom-0 w-px bg-ivory/90" style={{ left: "0%" }} />
            <span className="absolute t-label-strong bg-graphite-0/60 px-[calc(var(--u)*1)] py-[calc(var(--u)*0.5)]" style={{ top: "calc(var(--u) * 3)", left: "calc(var(--u) * 3)", fontSize: "calc(var(--u) * 1)" }}>
              Before
            </span>
            <span className="absolute t-label-strong bg-graphite-0/60 px-[calc(var(--u)*1)] py-[calc(var(--u)*0.5)]" style={{ top: "calc(var(--u) * 3)", right: "calc(var(--u) * 3)", fontSize: "calc(var(--u) * 1)" }}>
              After
            </span>
          </div>
        )}
        ...
        {/* Progress hairline */}
        <div className="absolute bottom-0 left-0 h-px bg-ivory/70 pointer-events-none" style={{ width: `${progress * 100}%` }} />
      </div>
    </div>
  );
}
...
function Caption({ kicker, title, sub, aspect }: { kicker: string; title: string; sub?: string; aspect: PresentAspect }) {
  return (
    ...
    >
      <div data-caption className="t-label" style={{ fontSize: "calc(var(--u) * 1)" }}>
        {kicker}
      </div>
      <div data-caption className="t-editorial text-ivory" style={{ fontSize: `calc(var(--u) * ${aspect === "9:16" ? 3.6 : 3})`, marginTop: "calc(var(--u) * 0.8)" }}>
        {title}
      </div>
      {sub && (
        <div data-caption className="text-warm-grey" style={{ fontSize: "calc(var(--u) * 1.05)", marginTop: "calc(var(--u) * 0.8)", letterSpacing: "0.04em" }}>
          {sub}
        </div>
      )}
    </div>
